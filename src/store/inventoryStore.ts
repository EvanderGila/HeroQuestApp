...
export const useInventoryStore = defineStore('inventory', () => {
  ...
  function setResources(res: GlobalResource[]) { resources.value = res }
  function setSharedInventory(inv: GlobalInventory[]) { sharedInventory.value = inv }
  function setLoading(status: boolean) { isLoading.value = status }

  return {
    resources, sharedInventory, isLoading, currentGold,
    setResources, setSharedInventory, setLoading
  }
})