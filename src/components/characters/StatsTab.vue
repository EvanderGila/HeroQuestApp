...
--v-activated-opacity: 0.06"
    ...
    >
      <div class="d-flex align-center">
        <VIcon
          :icon="
            currentAvailablePoints > 0 ? 'mdi-star-circle' : 'mdi-star-circle-outline'
          "
        ...
        <div>
          ...
          <div class="text-caption text-medium-emphasis">
            {{
              isEditing
                ? 'Ajusta tus stats y pulsa Guardar al terminar'
                : 'Gástalos para mejorar tus estadísticas'
            }}
          </div>
        </div>
      </div>
      ...
      <VCol cols="12" sm="6" v-for="stat in statsConfig" :key="stat.key" class="pa-1">
        ...
        >
          <div class="d-flex align-center">
            ...
            <div class="ml-2">
              <div
                class="text-caption text-medium-emphasis text-uppercase font-weight-bold lh-tight mb-1"
              >
                {{ stat.label }}
              </div>
              ...
              <div class="d-flex align-center">
                ...
                <div class="stat-value-wrapper">
                  <!-- 🔢 VALOR BASE -->
                  <div
                    class="text-h6 font-weight-black lh-1 stat-value"
                    :class="{
                  ...
                  >
                    {{
                      character[stat.key] +
                      (isEditing && stat.upgradable ? editedStats[stat.key] || 0 : 0)
                    }}
                  </div>
                </div>
                ...
                <span
                  ...
                  :class="
                    equipmentStats?.[stat.key as keyof typeof equipmentStats] < 0
                      ? 'text-error'
                      : 'text-success'
                  "
                >
                  ({{
                    equipmentStats?.[stat.key as keyof typeof equipmentStats] > 0
                      ? '+'
                      : ''
                  }}{{ equipmentStats?.[stat.key as keyof typeof equipmentStats] }})
                </span>
              </div>
            </div>
          </div>
    ...
    <VExpandTransition>
      <div
        v-if="isEditing"
        class="mt-4 pa-2 border-thin border-warning rounded-lg d-flex align-center justify-space-between flex-shrink-0 bg-warning-lighten-5"
      >
        <span
          class="text-caption font-weight-bold text-warning-darken-4 ps-2 d-flex align-center"
        >
          <VIcon icon="mdi-alert" class="me-1" color="warning" size="16" />
          Cambios sin guardar
        </span>
        <div class="d-flex gap-x-2">
          <VBtn size="small" variant="text" @click="cancelEdition">Cancelar</VBtn>
          <VBtn
            size="small"
            variant="flat"
            color="warning"
...
.upgradable-glow {
  ...
  box-shadow:
    0 0 12px rgb(var(--v-theme-primary), 0.1),
    inset 0 0 8px rgb(var(--v-theme-primary), 0.05) !important;
  animation: borderPulse 3s infinite ease-in-out !important;
}

@keyframes borderPulse {
  0% {
    border-color: rgb(var(--v-theme-primary), 0.3);
    box-shadow: 0 0 8px rgb(var(--v-theme-primary), 0.08);
  }

  50% {
    border-color: rgb(var(--v-theme-primary), 0.8);
    box-shadow: 0 0 16px rgb(var(--v-theme-primary), 0.35);
  }
...
.border-warning {
  border-color: rgb(var(--v-theme-warning), 0.3) !important;
}
...
.stat-value-active {
  color: #42a5f5;
  text-shadow: 0 0 8px rgb(66 165 245 / 25%);
}
...
.stat-assigned-badge {
  ...
  border: 1px solid rgb(66 165 245 / 25%);
  padding: 1px 4px;
  border-radius: 999px;
