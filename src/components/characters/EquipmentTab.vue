<template>
  ...
  <VRow dense>
    <VCol cols="6" sm="4" md="3" v-for="slot in availableSlots" :key="slot.key" class="pa-1">
  <div 
    class="hq-inventory-slot border-thin rounded-xl d-flex flex-column"
    :class="{
      'has-item': getEquippedItemInSlot(slot.key),
      'is-relic': getEquippedItemInSlot(slot.key)?.reliq /* 🌟 Tu campo real es 'reliq' */
    }"
  >
    <div class="hq-navbar-noise"></div>

    <template v-if="getEquippedItemInSlot(slot.key)">
      <div class="hq-slot-card-body h-100 d-flex flex-column justify-space-between position-relative">
        ...
        <div class="pa-2 d-flex flex-column justify-space-between flex-grow-1 z-index-2">
          ...
          <div class="hq-stats-row d-flex justify-center flex-wrap gap-x-1 mt-1">
            <span v-if="getEquippedItemInSlot(slot.key)?.atk_mod" class="hq-stat-mini atk">
              ⚔️{{ getEquippedItemInSlot(slot.key)!.atk_mod > 0 ? '+' : '' }}{{ getEquippedItemInSlot(slot.key)!.atk_mod }}
            </span>
            <span v-if="getEquippedItemInSlot(slot.key)?.def_mod" class="hq-stat-mini def">
              🛡️{{ getEquippedItemInSlot(slot.key)!.def_mod > 0 ? '+' : '' }}{{ getEquippedItemInSlot(slot.key)!.def_mod }}
            </span>
            <span v-if="getEquippedItemInSlot(slot.key)?.hp_mod" class="hq-stat-mini hp">
              ❤️{{ getEquippedItemInSlot(slot.key)!.hp_mod > 0 ? '+' : '' }}{{ getEquippedItemInSlot(slot.key)!.hp_mod }}
            </span>
            <span v-if="getEquippedItemInSlot(slot.key)?.mp_mod" class="hq-stat-mini mag">
              ✨{{ getEquippedItemInSlot(slot.key)!.mp_mod > 0 ? '+' : '' }}{{ getEquippedItemInSlot(slot.key)!.mp_mod }}
            </span>
          </div>
        </div>
    ...
    <template v-else>
      <div class="hq-slot-empty d-flex flex-column align-center justify-center pa-4 h-100 text-center">
        <VIcon :icon="slot.icon" size="24" class="hq-empty-icon mb-1" />
        <div class="hq-slot-tag font-weight-bold text-uppercase">{{ slot.label }}</div>
...
.gap-x-1 { column-gap: 4px !important; row-gap: 4px !important; }
.hq-section-title { color: #2196f3; letter-spacing: 0.08em; }

/* ── 🌌 CONTENEDOR CONTORNO DE RANURA ── */
.hq-inventory-slot {
  height: 100%;
  min-height: 175px;
  background: linear-gradient(180deg, rgba(20, 24, 35, 0.4) 0%, rgba(10, 11, 14, 0.6) 100%) !important;
...
.hq-inventory-slot.has-item {
  background: linear-gradient(180deg, rgba(14, 20, 32, 0.85) 0%, rgba(6, 7, 10, 0.95) 100%) !important;
...
.hq-inventory-slot.is-relic {
  ...
  box-shadow: 0 0 15px rgba(255, 179, 0, 0.15), inset 0 0 10px rgba(255, 179, 0, 0.05) !important;
}
...
.hq-stat-mini.atk { color: #ff8a80; border-color: rgba(255, 138, 128, 0.15); }
.hq-stat-mini.def { color: #81c784; border-color: rgba(129, 199, 132, 0.15); }
.hq-stat-mini.mag { color: #80deea; border-color: rgba(128, 222, 234, 0.15); }
