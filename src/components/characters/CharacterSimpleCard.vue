<template>
  <VCard class="fill-height d-flex flex-column border-thin elevation-2 character-card">
    ...
    <div class="d-flex align-center pa-4 border-bottom-thin gap-x-4">
      ...
      <div class="avatar-rpg-container flex-shrink-0 border-thin elevation-1 bg-grey-lighten-3">
        <VImg
          :src="character.img || character.races?.img || 'https://placehold.co/508x676?text=Hero'"
          cover
      ...
      <div class="flex-grow-1 d-flex flex-column justify-space-between h-100 overflow-hidden py-1">
        ...
        <div >
          <div class="text-caption text-medium-emphasis font-weight-medium text-truncate">
            {{ character.races?.name }} — <span class="font-weight-bold">{{ character.classes?.name }}</span>
          </div>
        </div>

        <!-- Barra de Experiencia Estilizada -->
        <div class="mt-1">
          <div class="d-flex align-center justify-space-between text-medium-emphasis text-uppercase font-weight-bold tracking-wide mb-1" style="font-size: 0.65rem;">
